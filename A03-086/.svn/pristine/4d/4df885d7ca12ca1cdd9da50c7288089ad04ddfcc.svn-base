var MIPopup = {
	...
	init: function() {
		...
		this.elePopupWindowBox.id = "popup-window-box";
		this.elePopupWindowBox.className = "popup-window-box";
		...
		if(MIGlobals.isRunningPhone){
			this.eleArrowUp.src = MIGlobals.contextRoot + "/META-INF/images/pop-arrow-up@2x.png";
		}
		else{
			this.eleArrowUp.src = MIGlobals.contextRoot + "/META-INF/images/pop-arrow-up.png";
		}
		this.elePopupWindowBox.appendChild(this.eleArrowUp);
		

		this.elePopupWindow = document.createElement("div");
		this.elePopupWindow.id = "popup-window";
		...
		this.eleButtonBaidu.className = "popup-window-button";
		this.eleButtonBaidu.setAttribute("data-href", "");
		this.eleButtonBaidu.setAttribute("data-target", "_blank");
		this.eleButtonBaidu.addEventListener(MIGlobals.eventTouchStart, MIPopup.wikiButtonOnTouchStart);
		this.eleButtonBaidu.addEventListener(MIGlobals.eventTouchEnd, MIPopup.wikiButtonOnTouchEnd);
		if (MIGlobals.isTouchSupported) {
			this.eleButtonBaidu.addEventListener(MIGlobals.eventTouchMove, MIPopup.wikiButtonOnTouchMove);
			this.eleButtonBaidu.addEventListener(MIGlobals.eventTouchCancel, MIPopup.wikiButtonOnTouchCancel);
		}
		...
		// this.eleButtonRw.addEventListener(MIGlobals.eventTouchStart, MIPopup.wikiButtonOnTouchStart);
		// this.eleButtonRw.addEventListener(MIGlobals.eventTouchEnd, MIPopup.wikiButtonOnTouchEnd);
		// if (MIGlobals.isTouchSupported) {
		// 	this.eleButtonRw.addEventListener(MIGlobals.eventTouchMove, MIPopup.wikiButtonOnTouchMove);
		// 	this.eleButtonRw.addEventListener(MIGlobals.eventTouchCancel, MIPopup.wikiButtonOnTouchCancel);
		// }
		...
		this.eleButtonHudong.className = "popup-window-button";
		this.eleButtonHudong.setAttribute("data-href", "");
		this.eleButtonHudong.setAttribute("data-target", "_blank");
		this.eleButtonHudong.addEventListener(MIGlobals.eventTouchStart, MIPopup.wikiButtonOnTouchStart);
		this.eleButtonHudong.addEventListener(MIGlobals.eventTouchEnd, MIPopup.wikiButtonOnTouchEnd);
		if (MIGlobals.isTouchSupported) {
			this.eleButtonHudong.addEventListener(MIGlobals.eventTouchMove, MIPopup.wikiButtonOnTouchMove);
			this.eleButtonHudong.addEventListener(MIGlobals.eventTouchCancel, MIPopup.wikiButtonOnTouchCancel);
		}
		...
		this.eleButtonWIKI.className = "popup-window-button";
		this.eleButtonWIKI.setAttribute("data-href", "");
		this.eleButtonWIKI.setAttribute("data-target", "_blank");
		this.eleButtonWIKI.addEventListener(MIGlobals.eventTouchStart, MIPopup.wikiButtonOnTouchStart);
		this.eleButtonWIKI.addEventListener(MIGlobals.eventTouchEnd, MIPopup.wikiButtonOnTouchEnd);
		if (MIGlobals.isTouchSupported) {
			this.eleButtonWIKI.addEventListener(MIGlobals.eventTouchMove, MIPopup.wikiButtonOnTouchMove);
			this.eleButtonWIKI.addEventListener(MIGlobals.eventTouchCancel, MIPopup.wikiButtonOnTouchCancel);
		}
		...
		*/
		...
		if (window.innerWidth<600) {
			this.eleArrowDown.src = MIGlobals.contextRoot + "/META-INF/images/pop-arrow-down@2x.png";
		}else{
			this.eleArrowDown.src = MIGlobals.contextRoot + "/META-INF/images/pop-arrow-down.png";
		}
	...
	makePopups: function(popups) {
		var ps = popups || document.querySelectorAll("span.popup-text");
	...
	makePopup: function (popupSpanElement) {
		...
		popupSpanElement.addEventListener(MIGlobals.eventTouchStart, MIPopup.popupSpanOnTouchStart);
		popupSpanElement.addEventListener(MIGlobals.eventTouchEnd, MIPopup.popupSpanOnTouchEnd);
		if (MIGlobals.isTouchSupported) {
			popupSpanElement.addEventListener(MIGlobals.eventTouchMove, MIPopup.popupSpanOnTouchMove);
			popupSpanElement.addEventListener(MIGlobals.eventTouchCancel, MIPopup.popupSpanOnTouchCancel);
		}
	},
	...
	popupSpanOnTouchCancel: function (e) {
		MIGlobals.removeClass(popupSpan, "popup-text-down");
		this.setAttribute("moved", "n");
	},

	popupSpanOnTouchStart: function (e) {
		MIGlobals.addClass(this, "popup-text-down");
		this.setAttribute("moved", "n");
	...
	popupSpanOnTouchEnd: function (e) {
		...
		if (MIPopup.currentPopupSpanId == null) {
			MIPopup.show(popupSpan, options);
		} else if (MIPopup.currentPopupSpanId == popupSpanId) {
			MIPopup.hide();
		} else {
			var popupWindowBox = MIPopup.elePopupWindowBox;
			popupWindowBox.style.opacity = "0";
			popupWindowBox.style.display = "none";
			MIPopup.currentPopupSpanId  = null;
			setTimeout(function(){
				MIPopup.show(popupSpan, options);
			}, 10);
		}
	...
	show: function  (elePopupSpan, options) {
		...
		if (!MIGlobals.isRunningPhone) {
			popupWindowBox.style.width="320px";
		}else{
...
//		this.eleButtonCloud.style.display = "none";
		...
		var defaultArrowX = ((popupW - MIPopup.arrowWidth) / 2);
		var deltaX = 0;
		...
		if(MIGlobals.isRunningPhone){
			MIPopup.minLeft = 15;			
			MIPopup.maxRight = window.innerWidth - MIPopup.minLeft;			
		}
		//log(MIPopup.minLeft)
		if (pageX >= (MIPopup.minLeft + popupW / 2) && pageX <= (MIPopup.maxRight - popupW / 2)) {
			
		} else if (pageX < (MIPopup.minLeft + popupW / 2)) {
			deltaX = (MIPopup.minLeft + popupW / 2) - pageX;
		} else if (pageX > (MIPopup.maxRight - popupW / 2)) {
			deltaX = -(pageX - (MIPopup.maxRight - popupW / 2));
		}

		popupWindowBox.style.left = (defaultWindowX + deltaX) + "px";
		if(MIGlobals.isRunningPhone){

		popupWindowBox.style.right = MIPopup.minLeft + "px";
		popupWindowBox.style.left = MIPopup.minLeft + "px";
		}
		...
		if (arrowX < MIPopup.popupWindowBorderRadius) {
			arrowX = MIPopup.popupWindowBorderRadius;
		} else if (arrowX > (popupW - MIPopup.popupWindowBorderRadius - MIPopup.arrowWidth)) {
			arrowX = (popupW - MIPopup.popupWindowBorderRadius - MIPopup.arrowWidth);
			// log("arrowX = " + arrowX);
		}
