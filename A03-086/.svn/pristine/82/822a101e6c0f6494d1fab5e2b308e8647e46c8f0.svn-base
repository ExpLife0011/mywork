 ...
 /** 将用户选择的答案和选择的内容发送给，客户端。
   ...
   function saveActivityData() {
   		...
   		if (true) {
 			...
 			for(var i=0,n=ques.length; i<n; i++){
 				...
 				if (ques[i].getAttribute('data-select-index')) {
 					userAnswers = ques[i].getAttribute('data-select-index').replace(/\d/g, function(n) {return table[n];});
 				}else{
 					userAnswers = " ";
 				}
 				dataContent[i].user_answers = [{
 					user_answer : userAnswers

 				}];
 				dataContent[i].right_answers = [{
 					right_answer : ques[i].parentElement.getAttribute('data-answer')
 				}];

 				// dataContent[i].right_answers = ques[i].parentElement.getAttribute('data-answer');
 				
 				if(!(MIGlobals.hasClass(radius[i],"downno") || MIGlobals.hasClass(radius[i],"downyes"))){
 					
 					dataContent[i].is_right = null;
 				}else{

 					if (MIGlobals.hasClass(radius[i],"downyes")) {
		     ...
		     var data2Post = {
		            ...
		            topic_right_count :document.querySelectorAll(".downyes").length,
		            enter_time:"",
		            exit_time:"",
		            topics: dataContent
		        };
 			...
 			for(var i=0,n=ques.length; i<n; i++){
 				...
 				dataContent[i].user_answers = [];
 				var checked = ques[i].querySelectorAll(".option-checked");
 				for (var j = 0; j < checked.length; j++) {
 					dataContent[i].user_answers[j] = {};
 					dataContent[i].user_answers[j].user_answer= checked[i].getAttribute('data-option-no');
 				}

 				var answer = ques[i].getAttribute('data-answer').split("-");
 				dataContent[i].right_answers = [];
 				for (var j = 0; j < answer.length; j++) {

 					dataContent[i].right_answers[j]={};
 					dataContent[i].right_answers[j].right_answer=answer[j];
 				}
 				
 				if(!(MIGlobals.hasClass(btnright[i],"book-btn-yes") || MIGlobals.hasClass(btnright[i],"book-btn-no"))){
 					dataContent[i].is_right = null;
 				}else{

 				if (MIGlobals.hasClass(btnright[i],"book-btn-yes")) {
 					dataContent[i].is_right ="Y";
	 				}else{
	 					dataContent[i].is_right ="N";
	 				}
        ...
        var data2Post = {
            ...
            topic_right_count :document.querySelectorAll(".book-btn-yes").length,
            enter_time:"",
            exit_time:"",
            topics: dataContent
        };
