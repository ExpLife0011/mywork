...
var MIPQuiz = {
	...
	popupSlotId : -1, // 当前弹出的槽位ID
	slotOptions: [], // 每个填空槽位中要显示的候选项列表。
					 // key - value 格式，key 为 slot 的 ID， value 为对应候选项的 id
					 // 在页面编写的时候，候选项和填空槽位的 ID 是不需要指定的，在初始化的时候会自动为其分配。
	//slotOptionMaxWidths : [], // 每个填空槽位选择了候选项之后的最大宽度。预先计算好，省却每次计算的消耗, key - value 格式，key 为槽位 ID，value 为整数，表示最大的宽度
	/** 初始化
	...
	init: function(options) {
		...
		for (var i = 0, n = slots.length; i < n; i++) {
			slots[i].id = "MIR-SLOT-" + i;
			slots[i].setAttribute("data-answer-id", "MIR-OPT-" + slots[i].getAttribute("data-answer-index"));
		}
		...
		setTimeout(function(e){
			MIPQuiz.h = window.innerHeight;
			MIPQuiz.w = window.innerWidth;
		}, 100);
		
		// 创建弹出层元素
		var popupLayer = document.createElement("DIV");
		popupLayer.id = "p-options";
		popupLayer.style.width = this.popupLayerWidth + "%";
		...
		for (var i = 0; i < this.optionCount; i++) {
			...
			// popupOption.innerHTML = document.getElementById("MIR-OPT-" + i).innerHTML;
			// popupOption.setAttribute("data-opt-id", "MIR-OPT-" + i);
		...
		for (var i = 0, n = slots.length; i < n; i++) {
			var slot = slots[i];
			var rightAnswerIndex = this.myArrayIndexOf(indexes, +(slot.getAttribute("data-answer-index")));
			...
			this.slotOptions[slot.id] = [];
			for (var j = fromIndex; j < (fromIndex + this.optionCount); j++) {
				this.slotOptions[slot.id][j - fromIndex] = indexes[j % indexes.length];
			}
			//console.log(slot.id);
			//console.log(this.slotOptions[slot.id]);

			MIGlobals.makeTouchableButton(slot);
			slot.addEventListener(MIGlobals.eventTouchEnd, function(e) {
		...
		document.addEventListener(MIGlobals.eventTouchEnd, function(e) {
			if (! MIGlobals.hasClass(e.target, "slot") && ! MIGlobals.hasClass(e.target, "opt") && ! MIGlobals.hasClass(e.target, "pop")) {
	...
	toggleStatus: function() {
		...
		var slots = document.querySelectorAll(".slot");
		for (var i = 0, n = slots.length; i < n; i++) {
			MIGlobals.removeClass(slots[i], "right");
			MIGlobals.removeClass(slots[i], "wrong");
			slots[i].setAttribute("data-choice", "");
			slots[i].style.webkitTransform = "rotateY(90deg)"
		}
		...
		setTimeout(function(){
			...
			for (var i = 0, n = slots.length; i < n; i++) {
				if (MIPQuiz.status === 1) {
					slots[i].innerHTML = document.getElementById(slots[i].getAttribute("data-answer-id")).innerHTML;
				} else {
					slots[i].innerHTML = "?";
				}
				slots[i].style.webkitTransform = "rotateY(0deg)"
			}	
			if (MIPQuiz.status === 0) {
				MIPQuiz.status = 1;
	...
	showPopup: function(slotId) {
		...
		for (var i = 0, n = options.length; i < n; i++) {
			...
			options[i].innerHTML = document.getElementById("MIR-OPT-" + this.slotOptions[slotId][i]).innerHTML;
			options[i].setAttribute("data-opt-id", "MIR-OPT-" + this.slotOptions[slotId][i]);
		}
		...
		this.popupLayerHeight = popupLayer.offsetHeight;

		
		if (top + this.popupLayerHeight > MIPQuiz.h) {
	...
	selectOption: function(optionId) {
		...
		if (this.popupSlotId === "-1") return;
		document.getElementById(this.popupSlotId).innerHTML = document.getElementById(optionId).innerHTML;
		document.getElementById(this.popupSlotId).setAttribute("data-choice", optionId);
		document.getElementById(this.popupSlotId).parentElement.setAttribute("data-select","y")

		// 检查是否全部答完
		var finished = true;
		var slots = document.querySelectorAll(".slot");
		for (var i = 0, n = slots.length; i < n; i++) {
			if ( ! slots[i].getAttribute("data-choice")) {
				finished = false;
				break;
			}
		}
		if (finished) {
			for (var i = 0, n = slots.length; i < n; i++) {
				var slot = slots[i];
				if (slot.getAttribute("data-answer-id") === slot.getAttribute("data-choice")) {
					MIGlobals.removeClass(slot, "wrong");
					MIGlobals.addClass(slot, "right");
				} else {
					MIGlobals.removeClass(slot, "right");
					MIGlobals.addClass(slot, "wrong");
				}
			}
		}
	},
