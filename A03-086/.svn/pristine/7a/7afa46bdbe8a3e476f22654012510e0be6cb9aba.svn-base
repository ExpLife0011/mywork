var MISPhone = {
 	...
 	itemDown: function(e){
 		...
 		if(MIGlobals.hasClass(eParent, 'multi-choice')){
 			...
 			if (MIGlobals.hasClass(Multiclect[e.target.getAttribute('data-index')],"sdown")) {
 				MIGlobals.removeClass(Multiclect[e.target.getAttribute('data-index')],"sdown");
 				eParent.setAttribute('data-select-index',"")
 			}else{
 				

 				MIGlobals.addClass(Multiclect[e.target.getAttribute('data-index')],"sdown");
 				var answer = document.querySelectorAll('.quiz-sbox')[MISPhone.currentIndex].querySelectorAll(".sdown");
 				var array = new Array;
 				for (var i = 0; i < answer.length; i++) {
 					array[i] = answer[i].getAttribute("data-index");
 				}
 				var indexes = array.toString().replace(/,/g,"-");
 			...
 			if(clickNo !== null){
				...
				// selectItem.style.backgroundColor = '#fff';
 	...
 	showAnswer: function(){
 		...
 		// else if(this.getAttribute('data-click') == '0'){
 			this.innerHTML = '查看答案';
