...
MIMark.prototype.createMarkSpan = function () {
	...
	if (this.userId == MIMarkManager.currentUserId) {
		objSpan.setAttribute("class", "mark noteicon");
 ...
 */
MIMark.prototype.createBookmark = function(markId, yPosition) {
	this.type = MIMark.TYPE_BOOKMARK;
	if (!markId) this.uuid = 'M-' + uuid.v4().toUpperCase();
	
	if (yPosition == -1) {
		this.pageTop = window.scrollY + 55;
		// New created bookmark, get the summary text.
		var topEle = this.findBookmarkableElement();
		if (topEle) {
			var s = topEle.textContent.replace(/\s/g, '');
			var title = document.querySelector('head > title');
			if (title) title = title.textContent;
			this.text = title + " : " + s.substring(0, 20);
		} else {
			var title = document.querySelector('head > title');
			if (title) {
...
MINoteBox.prototype.createNoteBoxElement = function() {
	...
	o.style.top = (this.pageTop - MIMark.noteImgTopOffset) + "px";
...
MINoteBox.prototype.deleteNoteBoxElement = function(e) {
...
var MIMarkManager = {
	...
	/** DEPRECATED 请求应用中的服务，获取应用中已经保存的高亮、笔记、书签数据
	 *
	 */

	 /*
	init: function() {
		this.currentUserId = MIGlobals.userId;

		var fileId = MIGlobals.getFileUuid();
		if (fileId.length == 0) {
			log("No file uuid detected, do not load marks");
			return false;
		}
		MIGlobals.ajax({
			url: "/" + MIGlobals.bookId + "/listMark.json",
			data: {fileId: fileId},
			success: function(response) {
				// console.log(response);
				for (var i = 0; i < response.length; i++) {
					var m = response[i];
					MIMarkManager.loadMark(m.markId, m.startOffset, m.endOffset, m.type, MIGlobals.userId);
				}
			}
		});
	},
	*/
	...
	loadMark: function(markId, startOffset, endOffset, type, userId, color) {
		log(MIGlobals.format("Create mark: id=%s, startOffset=%s, endOffset=%s, type=%s, color", markId, startOffset, endOffset, type, color));
		var id;
		try {
			if (markId && MIMarkManager.marks[markId]) {
				...
				var mark = new MIMark();
				if (type == MIMark.TYPE_BOOKMARK) {
					mark.createBookmark(markId, startOffset);
					for (var k in this.marks) {
						var tmpm = this.marks[k];
						if (tmpm.type == MIMark.TYPE_BOOKMARK) {
							if (Math.abs(tmpm.pageTop - mark.pageTop) < 20) {
								mark.uuid = "-1";
								break;
							}
						}
	...
	markAsNote: function(markId) {
		...
		if (m) {
			...
			var notedome =document.querySelectorAll("span.mark[data-mark-id='" + markId + "']");

			for (var i = 0; i < notedome.length; i++) {
				 if (m.userId == MIMarkManager.currentUserId) {
				 	notedome[i].setAttribute("class","mark noteicon");
				 }else{
				 	notedome[i].setAttribute("class","mark mark-others");
				 }
	...
	markAsHighlight: function(markId) {
		...
		if (m) {
			...
			for (var i = 0; i < notedome.length; i++) {
				 	...
				 	notedome[i].setAttribute("class","mark hightlight");
			...
			if (mark.type == MIMark.TYPE_HIGHLIGHT) {
				...
				if (noteBox) {
					...
					ret = {
						...
						idsInclude: markIds,
						noteIconCenterX: MIMark.noteImgCenterX,
						text: mark.text
					};
				} else {
...
function markSpanOnTouchStart(e) {
	...
	this.setAttribute("clickFrom", "content");
	...
	if (mark) {
		...
		if (mark.type == MIMark.TYPE_HIGHLIGHT) {
			...
			if (noteBox) {
				...
				url = MIGlobals.format("/mark?type=NOTE&ids=%s&top=%s&bottom=%s&pageTop=%s&pageBottom=%s&noteIconCenterX=%s&clickFrom=%s", 
					...
					noteBox.pageTop,
					noteBox.pageBottom,
					MIMark.noteImgCenterX,
					this.getAttribute("clickFrom"));
			} else {
				log("Can not find note box with mark id " + markId);
			}
		} else if (mark.type == MIMark.TYPE_BOOKMARK) {
			url = URLBuilder.buildURLString("/mark",  {
				"type" : "B",
				"ids"  : mark.uuid,
				"pageTop" : mark.pageTop,
...
function noteBoxOnTouchStart(e) {
	...
	this.setAttribute("data-moved", "n");
	this.setAttribute("clickFrom", "left");
}
...
function noteBoxOnTouchEnd(e) {
	...
	var url = MIGlobals.format("/mark?type=NOTE&ids=%s&top=%s&bottom=%s&pageTop=%s&pageBottom=%s&noteIconCenterX=%s&clickFrom=%s", 
